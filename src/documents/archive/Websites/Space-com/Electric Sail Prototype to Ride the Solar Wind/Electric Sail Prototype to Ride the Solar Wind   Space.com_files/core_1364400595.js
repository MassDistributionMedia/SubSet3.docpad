...
{return null;}
if((arr.itxt&&arr.itxt.arrayExtended)||!_.isArray(arr))
{return arr;}
else
...
{var removeFunc=$iTXT.core.Event.subscribe(this,type,handler);if(_.isArray(disposeArray))
...
return this;},router:function(scope){return function(evts){var i;if(_.isArray(evts)){for(i=0;i<evts.length;i++){evts[i]=[evts[i][0],$iTXT.core.Event.bind(scope,scope[evts[i][1]])];}}
...
{if(e&&e.stopPropagation)e.stopPropagation();else if(window.event)window.event.cancelBubble=true;}};var eventSplitter=/\s+/;$iTXT.on=function(events,callback,context){var calls,event,list;if(!callback)return this;events=events.split(eventSplitter);calls=this._callbacks||(this._callbacks={});while(event=events.shift()){list=calls[event]||(calls[event]=[]);list.push(callback,context);}
return this;};$iTXT.off=function(events,callback,context){var event,calls,list,i;if(!(calls=this._callbacks))return this;if(!(events||callback||context)){delete this._callbacks;return this;}
events=events?events.split(eventSplitter):$iTXT.core.Util.objKeys(calls);while(event=events.shift()){if(!(list=calls[event])||!(callback||context)){delete calls[event];continue;}
for(i=list.length-2;i>=0;i-=2){if(!(callback&&list[i]!==callback||context&&list[i+1]!==context)){list.splice(i,2);}}}
return this;};$iTXT.trigger=function(events){var event,calls,list,i,length,args,all,rest;if(!(calls=this._callbacks))return this;rest=[];events=events.split(eventSplitter);for(i=1,length=arguments.length;i<length;i++){rest[i-1]=arguments[i];}
...
{return!_.isArray(o)&&!_.isFunction(o)&&_.isObject(o)&&!o.itxtClass;},isClass:function(o)
{return _.isObject(o)&&o.itxtClass;},isURL:function(s,i)
{if('string'!=typeof s)
{return false;}
...
{var out='';if('function'!=typeof obj&&($iTXT.core.Util.isObject(obj)||_.isArray(obj)||$iTXT.core.Util.isClass(obj))&&!obj.nodeType)
...
{if(key&&params)
{var name=key;var qName=key;if(_.isArray(key))
...
{var node;if(!_.isString(str)){return str;}
