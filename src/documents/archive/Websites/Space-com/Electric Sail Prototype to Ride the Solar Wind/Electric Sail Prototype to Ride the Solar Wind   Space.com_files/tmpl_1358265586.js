...
{var newDims=this.events.onResize.apply(this,[w,h]);if(null!=newDims&&_.isArray(newDims))
...
{if(!ad)
{ad=this.advert;}
return'moatID='+ad.params.get('iono')+'|'+window.encodeURIComponent(ad.$A.kw)+'|'+ad.params.get('ipid')+'|'+ad.$A.did;}});};$iTXT.js.loader["$iTXT.tmpl.Html"]=true;$iTXT.tmpl.Html_Load=function(){var undefined;$iTXT.tmpl.Html=$iTXT.core.Class.create($iTXT.tmpl.ElementBase,{init:function(_options,ad,$super)
{var defOpts=$iTXT.core.Util.extend({id:$iTXT.tmpl.ElementIdentifier.get("itxtHtml")},_options);$super(defOpts,ad);if(_.isString(this.properties.src))
{this.rootElement.innerHTML=this.properties.src;}
else if(_.isFunction(this.properties.src))
...
{this.imageStyle.itxtSetStyle({width:"0px",height:"0px"});this.imageFailed=true;this.imageLoaded=true;this.reCalculateSize=true;this.properties.width=0;this.properties.height=0;$iTXT.core.$(document).itxtFire("$iTXT:tt:element:loaded",this.advert);if(this.events.error&&_.isFunction(this.events.error))
...
{if(null!=this.structure&&_.isArray(this.structure))
...
if(item.structure&&_.isArray(item.structure))
...
{this.templateBuilt=true;var b=$iTXT.core.Builder;this.rows=[];if(null!=this.structure&&_.isArray(this.structure))
