...
{this.logAV(ad,$iTXT.data.AdViewValue.ADVIEW_CPAV_CLOSED,0,o);}
...
{return;}}}
var redirect=qParams.redir;qParams.redir=undefined;var qString=this._buildQS(qParams);var override=false;if(_.isArray(ad.params.get('clicktag','')))
...
{if(a==this)
{return true;}
if(null!=this.childAdverts&&_.isArray(this.childAdverts))
...
var nameAttr=nd.getAttribute('name');if((_.isString(nd.id)&&'intellitxt'==nd.id.toLowerCase())||(nameAttr&&_.isString(nameAttr)&&'intellitxt'==nameAttr.toLowerCase())||assessment.isStartnode)
{if(this.metrics&$iTXT.debug.Util.ECH_TEXTNODES_DUMP)
{}
rA[rA.length]=nd;}}
...
{xV=xI[k];}
if(!_.isArray(xV)||$itxtUtil.inArray(xV,v))
{return;}
...
{for(var o in tO)
{var tOo=tO[o];if(_.isArray(tOo))
{cands=cands.concat(tOo);}}}}
if(cands.length>0&&$iTXT.data.Dom.findSelector(s,cands,1))
{var kN='is'+intt.substr(0,1).toUpperCase()+intt.substr(1);rO[kN]=true;match=true;}}
if(true==match)
{if(this._othHash[intt]&&this._othHash[intt][sz]&&_.isArray(this._othHash[intt][sz]))
{rO[intt]=this._othHash[intt][sz][0];}
delete i[intt];}}}}}
return rO;},add:function(sel,intt,oth)
{if((!$itxtUtil.isClass(sel)&&!_.isArray(sel))||'string'!=typeof intt)
{return;}
var sA=[];var oA=[];if(!_.isArray(sel))
{sA[0]=sel;}
else
{sA=sel;}
if(!_.isArray(oth))
...
{precision=0;}
var nA=[];if(!_.isArray(needle))
{nA[nA.length]=needle;}
else
{nA=needle;}
var hA=[];if(!_.isArray(haystack))
...
{var arr=this.get(n);if(_.isArray(arr))
{if(arr.length>i)
{return arr[i];}}
...
if($iTXT.glob.track&&$iTXT.glob.track[cat]&&_.isArray($iTXT.glob.track[cat])&&$iTXT.glob.track[cat].length>0)
